...
TestVersionLineNumber=`grep -nE 'version.test.*=' FYModuleA.podspec | cut -d : -f1`
sed -i "" "${TestVersionLineNumber}s/${TestVersionNumber}/${NewTestVersionNumber}/g" FYModuleA.podspec
echo "current test version is ${TestVersionNumber}, new version is ${NewTestVersionNumber}, line number is ${TestVersionLineNumber}"

GrayVersionLineNumber=`grep -nE 'version.gray.*=' FYModuleA.podspec | cut -d : -f1`
sed -i "" "${GrayVersionLineNumber}s/${GrayVersionNumber}/${NewGrayVersionNumber}/g" FYModuleA.podspec
echo "current gray version is ${GrayVersionNumber}, new version is ${NewGrayVersionNumber}, line number is ${GrayVersionLineNumber}"

VersionString=`grep -E 's.version.*=' FYModuleA.podspec`
...
VersionLineNumber=`grep -nE 's.version.*=' FYModuleA.podspec | cut -d : -f1`
sed -i "" "${VersionLineNumber}s/${VersionNumber}/${NewVersionNumber}/g" FYModuleA.podspec
...
pod repo push ${Repo} FYModuleA.podspec --verbose --allow-warnings --use-libraries --use-modular-headers
