...
Repo="FYModuleSpec"
RepoList=("FYModuleSpec" "FYModuleSpec_test" "FYModuleSpec_gray")
getRepo() {
    echo -e "\n"
    length=${#RepoList[@]}
...
DevelopVersionString=`grep -E 'version.develop.*=' FYModuleA_Extension.podspec`
DevelopVersionNumber=`tr -cd 0-9 <<<"$DevelopVersionString"`
NewDevelopVersionNumber=$DevelopVersionNumber

TestVersionString=`grep -E 'version.test.*=' FYModuleA_Extension.podspec`
TestVersionNumber=`tr -cd 0-9 <<<"$TestVersionString"`
NewTestVersionNumber=$TestVersionNumber

GrayVersionString=`grep -E 'version.gray.*=' FYModuleA_Extension.podspec`
...
if [ "$Repo" == "FYModuleSpec_test" ]; then
  NewTestVersionNumber=$(($TestVersionNumber + 1))
fi

if [ "$Repo" == "FYModuleSpec_gray" ]; then
  NewGrayVersionNumber=$(($GrayVersionNumber + 1))
fi

DevelopVersionLineNumber=`grep -nE 'version.develop.*=' FYModuleA_Extension.podspec | cut -d : -f1`
sed -i "" "${DevelopVersionLineNumber}s/${DevelopVersionNumber}/${NewDevelopVersionNumber}/g" FYModuleA_Extension.podspec
...
TestVersionLineNumber=`grep -nE 'version.test.*=' FYModuleA_Extension.podspec | cut -d : -f1`
sed -i "" "${TestVersionLineNumber}s/${TestVersionNumber}/${NewTestVersionNumber}/g" FYModuleA_Extension.podspec
echo "current test version is ${TestVersionNumber}, new version is ${NewTestVersionNumber}, line number is ${TestVersionLineNumber}"

GrayVersionLineNumber=`grep -nE 'version.gray.*=' FYModuleA_Extension.podspec | cut -d : -f1`
sed -i "" "${GrayVersionLineNumber}s/${GrayVersionNumber}/${NewGrayVersionNumber}/g" FYModuleA_Extension.podspec
echo "current gray version is ${GrayVersionNumber}, new version is ${NewGrayVersionNumber}, line number is ${GrayVersionLineNumber}"

VersionString=`grep -E 's.version.*=' FYModuleA_Extension.podspec`
...
VersionLineNumber=`grep -nE 's.version.*=' FYModuleA_Extension.podspec | cut -d : -f1`
sed -i "" "${VersionLineNumber}s/${VersionNumber}/${NewVersionNumber}/g" FYModuleA_Extension.podspec
...
pod repo push ${Repo} FYModuleA_Extension.podspec --verbose --allow-warnings --use-libraries --use-modular-headers
